...
export const getLang = (): Lang => {
  ...
  // 当没有配置语言，获取系统语言（统一转小写用于比较）
  const sysLang = navigator.language.toLowerCase()

  // 精确匹配，如 zh-cn 匹配 cn
  const exactMatch = localeList.find((lng) => lng.toLowerCase() === sysLang)
  if (exactMatch) return exactMatch

  // 前缀匹配，如 zh-hans-cn 匹配 cn，en 匹配 en
  const prefixMatch = localeList.find(
    (lng) => sysLang.startsWith(lng.toLowerCase()) || lng.toLowerCase().startsWith(sysLang),
  )
